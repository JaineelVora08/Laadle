...
def get_or_create_domain(name, domain_type='general'):
    """Create a DomainNode if it doesn't exist."""
    existing = DomainNode.nodes.filter(name=name)
    if existing:
        print(f"  ✓ Domain '{name}' already exists (uid={existing[0].uid})")
        return existing[0]
...
def seed():
    ...
    for u in pg_users:
        ...
        # Connect existing users to their domains
        if u.role == 'STUDENT':
            # Students interested in ML and Deep Learning
            for dname in ['Machine Learning', 'Deep Learning']:
                if dname in domain_nodes:
                    if not node.interested_in.is_connected(domain_nodes[dname]):
                        node.interested_in.connect(domain_nodes[dname], {
                            'priority': 1, 'current_level': 'beginner'
                        })
