...
# ── Common abbreviation / alias map for domain dedup ──
DOMAIN_ALIASES = {
    'ml': 'machine learning',
    'dl': 'deep learning',
...
def find_existing_domain(raw_text: str) -> 'DomainNode | None':
    """
    Try to find an existing DomainNode that matches *raw_text*.
    Search order:
      1. Case-insensitive exact match  (neomodel __iexact  →  Cypher toLower)
      2. Alias / abbreviation lookup
      3. Case-insensitive substring match (both directions)
    Returns the best DomainNode or None.
    """
    norm = _normalize_domain_name(raw_text)
    if not norm:
        return None

    # 1. Case-insensitive exact match via Cypher
    ...
    # 2. Check alias map
    canonical = DOMAIN_ALIASES.get(norm)
    if canonical:
        results2, _ = db.cypher_query(
            "MATCH (d:DomainNode) WHERE toLower(d.name) = $name RETURN d",
            {'name': canonical},
            resolve_objects=True,
        )
    ...
    # 3. Substring containment (match "cloud" → "Cloud Computing", etc.)
    #    Only if the input is >= 4 chars to avoid false positives
    if len(norm) >= 4:
        results3, _ = db.cypher_query(
            "MATCH (d:DomainNode) WHERE toLower(d.name) CONTAINS $name "
            "OR $name CONTAINS toLower(d.name) RETURN d LIMIT 1",
            {'name': norm},
            resolve_objects=True,
        )
        if results3:
...
class AddDomainView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # 4a. Pinecone semantic similarity (if available)
            if embedding:
            ...
            # 4b. Robust Neo4j name-based dedup (case-insensitive + alias + substring)
            if domain_node is None:
                try:
                    existing = find_existing_domain(raw_text)
                    if existing:
                        domain_node = existing
                        domain_node.popularity_score = (domain_node.popularity_score or 0) + 1
                        domain_node.save()
                        logger.info('Reusing existing domain: %s (uid=%s)', domain_node.name, domain_node.uid)
                except Exception as dedup_exc:
                    logger.warning('Domain dedup lookup failed: %s', dedup_exc)
