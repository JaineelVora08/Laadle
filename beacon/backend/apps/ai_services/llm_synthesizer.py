...
{{"groups": [{{"label": "short description", "senior_ids": ["id1", "id2"]}}]}}"""
    ...
    def generate_followup_questions(self, query_content: str, domain_id: str, historical_followups: list = None) -> list:
        ...
        Guarantees at least 1 FAQ: falls back to a pure-LLM generation if
        the RAG-enriched attempt returns nothing.
        Returns: list of question strings (max 3, min 1)
        """
        history_text = ""
...
{{"questions": ["question 1", "question 2", "question 3"]}}"""
        ...
        questions = self._call_followup_llm(prompt)
        if questions:
            return questions

        # Fallback: generate at least 1 FAQ using only the student question (no RAG context)
        fallback_prompt = f"""A student asked the following question. Generate 1 likely follow-up question they might ask next.
Keep it specific and directly related to the original question.
